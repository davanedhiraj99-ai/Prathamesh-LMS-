...
router.post('/', checkAuth(async (req, res) => {
  const { title, type, batchId, url, fileSize, duration, thumbnail } = req.body;
  ...
  try {
    const result = await pool.query(
      `INSERT INTO batch_content 
       (title, type, batch_id, url, file_size, duration, thumbnail) 
       VALUES ($1, $2, $3, $4, $5, $6, $7) 
       RETURNING *`,
      [title, type, batchId, url, fileSize || null, duration || null, thumbnail || null]
    );
